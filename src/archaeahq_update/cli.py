"""
archaeahq-update — find archaeal genomes newly deposited at NCBI, run the ArchaeaHQ curation
pipeline on them, recommend which ones should be added and build the next database version.

Installed:            archaeahq-update <command> ...
From a clone:         python3 archaeahq_update.py <command> ...

    archaeahq-update setup                      create/verify the tools and the CheckM2 database
    archaeahq-update fetch-db                   download ArchaeaHQ v1.0 from figshare → Archaea_HQ-v1.0/
    archaeahq-update check                      list new NCBI assemblies per kingdom (no download)
    archaeahq-update run                        full evaluation → two result tables
    archaeahq-update sketch-db --out DIR        build a reusable skani sketch of the database
...
# Where the Archaea_HQ-v* version folders live by default: $ARCHAEAHQ_RELEASES, else the current
# folder (the repository launcher sets the variable to the repository folder).
RELEASES_DEFAULT = os.environ.get("ARCHAEAHQ_RELEASES") or os.getcwd()


def prog() -> str:
    """How the user invokes the tool, for hints: `archaeahq-update` or `python3 archaeahq_update.py`."""
    argv0 = os.path.basename(sys.argv[0] or "")
    return f"python3 {argv0}" if argv0.endswith(".py") else "archaeahq-update"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="archaeahq-update", description=__doc__,
    ...
    def common_opts(sp, need_db=False):
        ...
        sp.add_argument("--releases-dir", default=RELEASES_DEFAULT,
                        help=f"folder holding the database versions {C.RELEASE_PREFIX}<major>.<minor>/ "
                             f"(default: $ARCHAEAHQ_RELEASES, else the current folder)")
...
def stage_report(ctx: Context, listing, downloads, meta, qual, red, rna_counts, out_dir: Path) -> None:
    ...
    if ctx.args.sankey:
        try:
            from . import sankey_generic
            C.run_cmd([sys.executable, str(sankey_generic.__file__), str(sankey_in),
...
def _fetch_next_steps(ctx: Context) -> None:
    ...
    ui.kv("1. what is new", f"{prog()} check")
    ui.kv("2. evaluate it", f"{prog()} run --threads 24")
    ui.kv("3. next version", f"{prog()} release")
    ui.info(f"the tools are installed with `{prog()} setup` if you have not done it yet")
...
def cmd_check(ctx: Context) -> int:
    ...
    ui.info(f"run `{prog()} run` to evaluate them")
    return 0
...
def cmd_release(ctx: Context) -> int:
    ...
    if a.table:
        ...
        if run_dir is None or not run_dir.is_dir():
            raise SystemExit(f"no finished run found in {ctx.runs_dir} — run `{prog()} run` first or pass --table")
...
if __name__ == "__main__":  # python -m archaeahq_update.cli
    sys.exit(main())
